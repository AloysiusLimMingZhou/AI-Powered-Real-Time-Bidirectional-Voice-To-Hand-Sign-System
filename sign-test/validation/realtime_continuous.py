...
PREDICTION_INTERVAL = 10  # Predict every N frames (for smoother performance)
...
class ContinuousSignRecognizer:
    ...
    def predict(self):
        ...
        landmarks_array = np.array(list(self.landmark_buffer)).astype(np.float32)
    ...
    def run(self):
        ...
        while True:
            ...
            self.frame_count += 1
            if self.frame_count % PREDICTION_INTERVAL == 0 and len(self.landmark_buffer) >= WINDOW_SIZE:
                result = self.predict()
